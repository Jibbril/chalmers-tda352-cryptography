...
- **Closure**: The outcome of $\oplus$ will always be in $G$. So $g \oplus h \in G, \quad \forall g,h \in G$.
- **Associativity**: $(g \oplus h) \oplus k = g \oplus (h \oplus k)$
- **Identity**: There exists an element $e \in G$ such that $e \oplus g = g \oplus e = g, \quad \forall g \in G$
...
One this to note about Euler's totient function is that if $N$ is the product of two primes $p,q$ then the function simplifies to 
...
Let $G$ be a cyclic group of order $q$ ($q$ is an $n$-bit long prime), and $g$ be a generator or $G$. The DLA states that it is computationally infeasible for any efficient attacker to find the exponent $x$ such that $`g^x = h`$ for some $h \in G$. Formally we have 
